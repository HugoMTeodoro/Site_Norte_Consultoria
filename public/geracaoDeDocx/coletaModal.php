...
DATE_FORMAT(dt_homologacao,'%d/%m/%Y') as datarat
 ...
 FROM modalidade where codigo_processo='$codigo_proc'";
$inex = $connection->query($sqlQuery);
if ($inex->num_rows > 0) {
    while ($row = $inex->fetch_assoc()) {
    ?>
        <form name="myform" action="../geracaoDeDocx/geraModal.php" method="POST">
            ...
            <input type="hidden" name="modalidade" value="<?php echo $row["modalidade"] ?>">
